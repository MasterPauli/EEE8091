...
//Writer:  Group 1
//Second versions
//19.11.30改动说明：增37行，82-87，211-251.结束已知内容。
////////////////////////////////////////////////////////////////////////////////////////
#include "setup.h"
#include <filters.h>
...
float x6I_prev=0.0;//output of symbol sync
float x6Q_prev=0.0;//output of symbol sync
float dI;
float dQ;
char data[72];
    		...
    		unsigned int * sample_out=src_pointer[int_cntr];
     		...
     		for(i=0;i<NUM_SAMPLES/2;i++)
     		{
				...
				if(Nc0 == Nc1)
				{
    				...
    				dI=x6I*x6I_prev + x6Q*x6Q_prev;
    				dQ=x6I_prev*x6Q + x6I*x6Q_prev;
				...
				*/
				sample_out[2*i+1]=(unsigned int)(((int)(x4I*scale_out))>>8);	// DAC 1				
     		 	sample_out[2*i]=(unsigned int)(((int)(x4Q*scale_out))>>8);	// DAC 2
     		}
